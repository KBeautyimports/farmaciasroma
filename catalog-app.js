...
function openModal(idx) {
  ...
  if (!p.image || p.image === 'nan') {
    ...
    document.querySelector('.modal-img-wrap').innerHTML = `<button class="modal-close" onclick="closeModalBtn()">✕</button><div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:4rem;">${BRAND_INFO[p.brand]?.emoji || '🌸'}</div>`;
  }
...
// KIOSK_MODE true (tablet): buttons play the video/post inline, right
//   in this modal — the shopper never leaves the shared kiosk screen.
// KIOSK_MODE false (qr.html): buttons are real links straight to the
//   TikTok/Instagram URL. On someone's own phone, tapping a real link
//   to tiktok.com or instagram.com lets iOS/Android's built-in
//   Universal Links open that post directly in the native app (falling
//   back to the mobile site if it isn't installed) — nothing extra to
//   configure, it just needs to be a plain <a href> and not a JS
//   navigation. That's a better viewing experience than a small inline
//   embed on a personal device, at the cost of leaving the catalog.
function renderReviews(reviews) {
  const wrap = document.getElementById('modal-reviews-wrap');
  const btnRow = document.getElementById('modal-reviews-buttons');
  const embed = document.getElementById('modal-reviews-embed');
  embed.innerHTML = '';
  ...
  btnRow.innerHTML = reviews.map((r, i) => {
    const icon = r.platform === 'tiktok' ? '🎵' : '📷';
    const label = r.platform === 'tiktok' ? 'TikTok' : 'Instagram';
    if (KIOSK_MODE) {
      return `<button class="review-btn ${r.platform}" id="review-btn-${i}" onclick="showReview(${i}, '${r.platform}', '${r.url}')">${icon} Ver en ${label}</button>`;
    }
    return `<a class="review-btn ${r.platform}" href="${r.url}" target="_blank" rel="noopener">${icon} Ver en ${label}</a>`;
  }).join('');
}

function showReview(i, platform, url) {
  const embed = document.getElementById('modal-reviews-embed');
  document.querySelectorAll('.review-btn').forEach(b => b.classList.remove('playing'));
  document.getElementById(`review-btn-${i}`).classList.add('playing');

  if (platform === 'tiktok') {
    embed.innerHTML = `<blockquote class="tiktok-embed" cite="${url}" style="max-width:325px;min-width:280px;"><section></section></blockquote>`;
  } else {
    embed.innerHTML = `<blockquote class="instagram-media" data-instgrm-permalink="${url}" style="width:100%;max-width:400px;margin:0;"></blockquote>`;
  }
...
function toggleConcern(value, el) {
  ...
  document.getElementById('next-1').disabled = quizAnswers.concerns.length === 0;
}
